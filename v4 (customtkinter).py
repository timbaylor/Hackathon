...
def add_profile():
    ...
    profile_window = ctk.Toplevel(app)
    profile_window.title("Add Profile")

    global profile_name_var
    profile_name_var = ctk.StringVar()
    profile_name_entry = ctk.CTkEntry(profile_window, textvariable=profile_name_var)

    ctk.CTkLabel(profile_window, text="Profile Name:").grid(row=0, column=0, padx=10, pady=10)
    profile_name_entry.grid(row=0, column=1, padx=10, pady=10)
    ctk.CTkButton(profile_window, text="Save Profile", command=add_profile_data).grid(row=1, column=0, columnspan=2, pady=10)
...
def open_water_usage_inputs_window():
    ...
    water_usage_inputs_window = ctk.Toplevel(app)
    water_usage_inputs_window.title("Water Usage Inputs")

    inputs_frame = ctk.CTkFrame(water_usage_inputs_window, text="Water Usage Inputs")
    ...
    for i, text in enumerate(inputs):
     ctk.CTkLabel(inputs_frame, text=text).grid(row=i, column=0, padx=10, pady=10)
    ...
    shower_minutes_var = ctk.StringVar()
    shower_minutes_entry = ctk.CTkEntry(inputs_frame, textvariable=shower_minutes_var)
    shower_minutes_entry.grid(row=0, column=1, padx=10, pady=10)

    showers_per_week_var = ctk.StringVar()
    showers_per_week_entry = ctk.CTkEntry(inputs_frame, textvariable=showers_per_week_var)
    showers_per_week_entry.grid(row=1, column=1, padx=10, pady=10)

    toilet_flushes_var = ctk.StringVar()
    toilet_flushes_entry = ctk.CTkEntry(inputs_frame, textvariable=toilet_flushes_var)
    toilet_flushes_entry.grid(row=2, column=1, padx=10, pady=10)

    faucet_minutes_var = ctk.StringVar()
    faucet_minutes_entry = ctk.CTkEntry(inputs_frame, textvariable=faucet_minutes_var)
    faucet_minutes_entry.grid(row=3, column=1, padx=10, pady=10)

    dishwasher_loads_var = ctk.StringVar()
    dishwasher_loads_entry = ctk.CTkEntry(inputs_frame, textvariable=dishwasher_loads_var)
    dishwasher_loads_entry.grid(row=4, column=1, padx=10, pady=10)

    washing_machine_loads_var = ctk.StringVar()
    washing_machine_loads_entry = ctk.CTkEntry(inputs_frame, textvariable=washing_machine_loads_var)
    washing_machine_loads_entry.grid(row=5, column=1, padx=10, pady=10)

    calculate_button = ctk.CTkButton(inputs_frame, text="Calculate", command=on_calculate)
    calculate_button.grid(row=len(inputs) + 1, column=0, pady=10)


users = []

profile_frame = ctk.CTkFrame(app, text="Profile Management")
profile_frame.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")

name_var = ctk.StringVar()
name_label = ctk.CTkLabel(profile_frame, textvariable=name_var)
name_label.grid(row=0, column=1, padx=10, pady=10)

profile_select = ctk.CTkComboBox(profile_frame, state='readonly')
profile_select.grid(row=1, column=1, padx=10, pady=10)
profile_select.bind('<<ComboboxSelected>>', on_profile_select)

add_profile_button = ctk.CTkButton(profile_frame, text="Add Profile", command=add_profile)
add_profile_button.grid(row=2, column=1, pady=10)

delete_profile_button = ctk.CTkButton(profile_frame, text="Delete Profile", command=delete_profile)
delete_profile_button.grid(row=3, column=1, pady=10)

view_profile_button = ctk.CTkButton(profile_frame, text="View Profile", command=view_profile)
view_profile_button.grid(row=7, column=1, pady=10)

graph_button = ctk.CTkButton(profile_frame, text="Graph", command=show_graph)
